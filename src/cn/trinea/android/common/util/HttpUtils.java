...
public class HttpUtils {
	 ...
	 *            设置从主机读取数据超时（单位：毫秒）
	 */
	public static String getWebSourceByUrl(String urlString,
			int readTimeOutMillis) {
		if (StringUtils.isEmpty(urlString)) {
			return urlString;
		} else {
			InputStream is = ImageUtils.getInputStreamFromUrl(urlString,
					readTimeOutMillis);
			if (is != null) {
				...
				String html = new String(arr); // 获取网页源码
				sb_result.append(html);
